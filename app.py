...
def fetch_details(movie_id):
    try:
        ...
        if not trailer_url:
            trailer_url = f"https://www.youtube.com/results?search_query={data.get('title')}+official+trailer"

        return poster, rating, year, trailer_url, data.get('title')

    except:
        return "https://via.placeholder.com/500x750?text=Error", 0, "N/A", "https://www.youtube.com", "Movie"
...
def recommend(movie):
    ...
    for i in movie_list:
        ...
        poster, rating, year, trailer, title = fetch_details(movie_id)
...
if st.button('Recommend'):
    ...
    for col, movie in zip(cols, recommendations):
        with col:
            st.image(movie['poster'], use_container_width=True) # Forces image to fit
